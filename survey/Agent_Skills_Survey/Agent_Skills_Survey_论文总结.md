...
|------|------------------|------|------|
| **基于文本（Text-based）** | 参考、示例、模板、评分量表 | 落地与复用，无可执行依赖 | 执行不确定 |
| **基于代码（Code-based）** | 脚本、辅助函数、笔记本、包装器 | 可重复、操作确定性强 | 引入版本控制/测试/依赖管理成本 |
| **混合（Hybrid）** | 文本 + 可执行 | 保留可解释性 + 支持确定性执行 | 协调负担最高（需保持一致性） |
