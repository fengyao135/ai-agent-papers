...
```
规范(SKILL.md等) → 发现(注册表+语义匹配) → 渐进式披露(名称→摘要→完整指南)
  → 执行绑定(连接工具/API/协议) → 组合(串行/并行/条件/递归)
```
...
```
记忆 ──经验蒸馏──→ 技能    （重复轨迹被聚类、抽象、提升为技能工件）
技能 ──执行记录──→ 记忆    （每次执行的痕迹、失败、改进被持久化）
技能 ──能力调用──→ 协议    （技能通过协议接口跨越从抽象到受治理动作的边界）
协议 ──能力生成──→ 技能    （标准化接口扩展可编写新技能的表面）
记忆 ──策略选择──→ 协议    （历史成功率、用户偏好影响协议路径选择）
协议 ──结果同化──→ 记忆    （每次协议交互产生的状态被规范化写入记忆）
```
